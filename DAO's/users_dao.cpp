...
unique_ptr<user_info> users_dao::find_by_id(int id)
{
    ...
    const char *find_by_id_query = ""
                                   "SELECT user_id, username, user_email, user_password FROM users WHERE user_id = ?";
...
unique_ptr<user_info> users_dao::find_by_email(const string &email)
{
    ...
    const char *find_by_email_query = ""
                                      "SELECT user_id, username, user_email, user_password FROM users WHERE user_email = ?";
...
vector<user_info> users_dao::find_all()
{
    vector<user_info> users;

    sqlite3_stmt *stmt;

    const char *find_all_query = ""
                                 "SELECT user_id, username, user_email, user_password FROM users";

    int operation_code = sqlite3_prepare_v2(db_connection->handle_getter(), find_all_query, -1, &stmt, nullptr);
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        users.push_back(user_info(user_id, user_email, user_password, username));
    }
...
void users_dao::create_user(const string &username, const string &email, const string &password)
{
    sqlite3_stmt *stmt;
...
bool users_dao::validate_credentials(const string &email, const string &password)
{
    sqlite3_stmt *stmt;

    const char *check_query = ""
                              "SELECT user_email, user_password FROM users WHERE user_email = ? AND user_password = ?";


    int operation_code = sqlite3_prepare_v2(db_connection->handle_getter(), check_query, -1 , &stmt, nullptr);
